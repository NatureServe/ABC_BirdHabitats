...
## Input Variables
hexgrid = r"S:\Projects\ABC\y2022\Pro\Draft\QC_Efforts\QC_Efforts\Default.gdb\hex_SEextract_343sqmi_3" ## UPDATE: with entire 343 hex grid, or extract of area of interest
WorkingHabitat = r"S:\Projects\ABC\y2022\Workspace\Hannah_Ceasar\FinalMapAssembly_HC.gdb\WorkingHab_SEextract" ## UPDATE: with entire habtiat layer or extract of area of interest
...
hexid_list = []
with arcpy.da.SearchCursor(TabArea_out, ["summary_hex_l1_id", "Value"]) as cursor:
    for row in cursor:
...
for index in hexid_list:
    hexid = index[0]
    habcode = index [1]
    print("working on "+ str(habcode)+ " " + str(hexid))
    
    # select hexids for each habitat code
    w_clause = "Value = {}".format(habcode)
    
    #hexgridselection = arcpy.management.SelectLayerByAttribute(hexgrid, "NEW_SELECTION", where_clause = w_clause)
    table_export = arcpy.conversion.TableToTable(TabArea_out, out_path = intWorkspace, out_name =f"hex_{habcode}", where_clause= w_clause)
